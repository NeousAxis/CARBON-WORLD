...
/**
 * TopCountriesBurnCard — Server Component
 *
 * Shows top 5 countries with most BURN decisions (positive actions) over 7 days.
 *
 * Example props for smoke-test:
 * <TopCountriesBurnCard countries={[
 *   { country: "Brazil", count: 3, total_amount: 8000000 },
 *   { country: "Germany", count: 2, total_amount: 4500000 },
 * ]} />
 */
export function TopCountriesBurnCard({ countries }: TopCountriesBurnCardProps) {
      ...
      {items.length === 0 ? (
        ...
        <div className="flex flex-col">
          {items.map((item, i) => (
            <Link
              key={item.country}
              href={`/events?country=${encodeURIComponent(item.country)}&decision=BURN&since=7d`}
              ...
              title={`See the ${item.count} BURN events from ${item.country}`}
            >
              {/* Rank */}
              <span
